...
import { getMessaging, getToken, onMessage } from 'https://www.gstatic.com/firebasejs/10.1.0/firebase-messaging.js';
  ...
  function getRegisterToken() {
      if ('serviceWorker' in navigator) {
          navigator.serviceWorker.register('firebase-messaging-sw.js?v=11')
            .then((registration) => {
  ...
  function saveToken(currentToken) {
    ...
    jQuery.ajax({
      ...
      type: "post",
      url: custom_domain_base+"/requests/subscription/"+user_id,
    ...
    $.ajax({ url:'https://json.geoiplookup.io/', type: "GET", dataType: "json",
      ...
      success: function(data_resp){
          ...
          jQuery.ajax({
            data: data_resp,
            type: "post",
            url: custom_domain_base+"/requests/clicked",
            success: function(result) {
              console.log(result);
            }
          });
...
document.addEventListener("DOMContentLoaded", function(event) {
  ...
  function open_popup_subscrabe(user_id, is_swfile) {
    ...
    http.open("GET", custom_domain_base+"/requests/popup/"+user_id+"/"+is_swfile);
